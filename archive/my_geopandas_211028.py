...
#     ''' ??? not used. I wanted to use this to reduce the points in the below shape file, but resulted in overlaps and gaps. The reduction in file size was also fairly small '''
#     for file in ['LocalGovernment','GovernmentRegion','GeologicalProvince','State']:

#         gdf = gpd.read_file(os.path.join(self.regions_dir,'%s.shp'%(file)))

#         gdf['geometry'] = gdf['geometry'].apply(lambda x: temp_clean_multipolygon_by_bearing(x))

#         gdf.to_file(os.path.join(self.regions_dir,'%s_B.shp'%(file)))



# def temp_clean_multipolygon_by_bearing(geom):
#     ''' ??? not used. used to reduce points in polygons that are already in shaptial form. It will also convert Polygons to Multipolygons '''
...
#     return MultiPolygon(multi_lst)



    # path = os.path.join(self.input_dir,'temp','temp.csv')

    # # r_path = os.path.join(self.input_dir,'temp','raw.csv')
    # e_path = os.path.join(self.input_dir,'temp','edit.csv')

    # df = pd.read_csv(path)

    # df['WKT'] = df['WKT'].apply(lambda x: clean_multipolygon_by_bearing(x))
    ...
    # multipolygon_geom = clean_multipolygon_by_bearing(gdf.iloc[0]['WKT'])

    # print(multipolygon_geom)

    # val = gdf.iloc[0]['WKT']

    # # print(val[0].exterior.coords)
    # # print(val[0].interiors[0].coords)
    ...
    #         poly_lst.append(int_pnt_lst)
    #     if len(poly_lst) == 0:
    #         multi_lst.append(Polygon(ext_pnt_lst))
    #     else:
    #         multi_lst.append(Polygon(ext_pnt_lst,poly_lst))

    # multipolygon_geom = MultiPolygon(multi_lst)


    # df = df[df['TITLE_NAME'] == 'AC/P36']
    # df.to_csv(r_path,index=False)

    # multipolygon_wkt = wkt.dumps(multipolygon_geom)
    # new_df = df
    # new_df['WKT'] = multipolygon_wkt
    # new_df.to_csv(e_path,index=False)
    # print('done')




# pnt_lst = poly.exterior.coords[:2]
#         for latlng in poly.exterior.coords[2:]:
#             pnt_a = pnt_lst[-2]
#             pnt_b = pnt_lst[-1]
#             pnt_c = latlng
#             bear_a = calculate_initial_compass_bearing(pnt_a, pnt_b)
#             bear_b = calculate_initial_compass_bearing(pnt_b, pnt_c)
#             # print('{:.20f}'.format(abs(bear_a - bear_b)))
#             if (abs(bear_a - bear_b)) < 0.05:
#                 pnt_lst.pop()
#             pnt_lst.append(latlng)

#         ext_pnt_lst = pnt_lst

    # print(multipolygon_wkt)



        # if val[0].interiors:
    #     ext_int_lst += val[0].interiors

    # print(Polygon(ext_int_lst))
    ...
    # print(val[0].exterior.coords[0])
    # print(val[0].exterior.coords[1])



    # print(calculate_initial_compass_bearing(lyon, paris))
    ...
    # target = None
    # nex = None
    # for x in val[0].exterior.coords:
    #     count += 1
    ...
    # print(haversine(lyon, paris, Direction.WEST))
    ...
    # print(val[0].interiors)
    # for x in val[0].interiors:
    #     print(x)
