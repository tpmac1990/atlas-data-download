...
from .directories_files import fileExist, getJSON
...
def tenement_format(self,df):
    df.rename(columns={'NEW_ID':'ind'},inplace=True)
    new_df = df[['ind','typ_id','status_id','state_id','shore_id','lodgedate','startdate','enddate']].copy()
...
def format_file_to_combine(self,file,configs):
    ...
    try:
        df = pd.read_csv(os.path.join(self.vba_dir,('%s.csv')%(file)), low_memory=self.configs[file]['preformat']['low_memory']) # ,engine='python' 
...
def format_site_file_to_combine(self,file,configs):
    ''' formates and assigns the necessary fields from the raw data file to output files '''
    try:
        df = pd.read_csv(os.path.join(self.vba_dir,('%s_WKT.csv')%(file)), low_memory=self.configs[file]['preformat']['low_memory']).fillna('')
    except pd.errors.EmptyDataError:
        print(f'## Empty file: {file}')
        return
...
def finalize_missing_data(self):
    ...
    # # Title type
    # # finds the closest match, but there can be a large margin of error due lo a lack of state, group sorting and the fact that the type can be derived from with the fname or code value
    # TenType_df = pd.read_csv(os.path.join(self.output_dir,'core',"TenType.csv"),engine='python')
    # all_miss_tentype_df = miss_df.query('FIELD == "TITLE_TYPE"')
    # miss_tentype_df = all_miss_tentype_df[['VALUE']].drop_duplicates()
    # tentype_lst = miss_tentype_df['VALUE'].to_list()
    # matched_tentype_df = find_similar_values(miss_tentype_df,TenType_df,'fname',tentype_lst)
    # final_tentype_df = all_miss_tentype_df.merge(matched_tentype_df,left_on='VALUE',right_on='ORIGINAL',how='left').drop('VALUE',1)

    # # Site type
    # OccType_df = pd.read_csv(os.path.join(self.output_dir,'core',"OccType.csv"),engine='python')
    # all_miss_occtype_df = miss_df.query('FIELD == "SITE_TYPE"')
    # miss_occtype_df = all_miss_occtype_df[['VALUE']].drop_duplicates()
    # occtype_lst = miss_occtype_df['VALUE'].to_list()
    # matched_occtype_df = find_similar_values(miss_occtype_df,OccType_df,'original',occtype_lst)
    # final_occtype_df = all_miss_occtype_df.merge(matched_occtype_df,left_on='VALUE',right_on='ORIGINAL',how='left').drop('VALUE',1)

    # # Holder
    # # find close matches for the missing holder names write to csv file
    # Holder_df = pd.read_csv(os.path.join(self.output_dir,'core',"Holder_pre.csv"),engine='python')
    # Holder_df = Holder_df[['name']]
    # all_miss_companies_df = miss_df.query('FIELD == "HOLDER"')
    # miss_companies_df = all_miss_companies_df[['VALUE']]
    # companies_lst = miss_companies_df['VALUE'].to_list()
    # matched_holder_df = find_similar_values(miss_companies_df,Holder_df,'name',companies_lst)
    # final_holder_df = all_miss_companies_df.merge(matched_holder_df,left_on='VALUE',right_on='ORIGINAL',how='left').drop('VALUE',1)

    # # Materials
    # Material_df = pd.read_csv(os.path.join(self.output_dir,'core',"Material.csv"),engine='python')
    # materials_R_df = pd.read_csv(os.path.join(self.output_dir,'convert','occurrence',"Materials_R.csv"),engine='python')
    # materials_R_df = materials_R_df[~materials_R_df['match'].isnull()]
    # # match the raw material codes with their formatted name
    # mat_raw_format_df = materials_R_df.merge(Material_df,on='code',how='left')
    # all_miss_materials_df = miss_df[miss_df['FIELD'].str.contains('MATERIAL')]
    # miss_materials_df = all_miss_materials_df[['VALUE']].drop_duplicates()
    # materials_lst = miss_materials_df['VALUE'].to_list()
    # matched_materials_df = find_similar_mat_values(miss_materials_df,materials_R_df,'match',materials_lst,mat_raw_format_df)
    # final_materials_df = all_miss_materials_df.merge(matched_materials_df,left_on='VALUE',right_on='ORIGINAL',how='left').drop('VALUE',1)

    # # Names
    # # no need to find similar values, only formats
    # miss_names_df = miss_df[miss_df['FIELD'] == 'NAME']
    # final_names_df = miss_names_df.rename(columns={'VALUE':'ORIGINAL'})
    # final_names_df['LIKELY_MATCH'] = final_names_df['ORIGINAL'].apply(lambda x: x.title())

    # # Status
    # # get all the core options and insert them as matches so the correct one can easily be selected and inserted as the LIKELY option
    # columns_lst = ['MATCH_%s'%(x) for x in list('ABCDEFGHIJKLMNOPQRSTUVWXYZ')]
    # final_tenstatus_df = all_core_options_as_matches(self,miss_df,'TenStatus','TITLE_STATUS','original',columns_lst)
    # final_occstatus_df = all_core_options_as_matches(self,miss_df,'OccStatus','SITE_STATUS','original',columns_lst)
    # final_occsize_df = all_core_options_as_matches(self,miss_df,'OccSize','SIZE','name',columns_lst)

    # # concatenate all the df's
    # complete_df = pd.concat((final_holder_df,final_materials_df,final_names_df,final_tenstatus_df,final_occstatus_df,final_tentype_df,final_occtype_df,final_occsize_df))
    ...
    # complete_df = miss_df[miss_df['FIELD'].isin(['TITLE_STATUS','SITE_STATUS','TITLE_TYPE','SITE_TYPE'])].rename(columns={'VALUE':'ORIGINAL'})

    # # Holder
    # final_df = find_similar_values(miss_companies_df,Holder_df,'name',lst)
    # # find close matches for the missing holder names write to csv file
    # Holder_df = pd.read_csv(os.path.join(self.output_dir,'core',"Holder_pre.csv"),engine='python')
    # Holder_df = Holder_df[['name']]
    # miss_companies_df = miss_df.query('FIELD == "HOLDER"')[['VALUE']]
    # if miss_companies_df.empty:
    #     final_df = pd.DataFrame([],columns=['ORIGINAL','LIKELY_MATCH','MATCH_A','MATCH_B','MATCH_C','MATCH_D','MATCH_E'])
    # else:
    #     final_df = pd.DataFrame()
    #     lst = miss_companies_df['VALUE'].to_list()

    #     for i in lst:
    #         # get ratio for each value in the holder df and then return the top 5
    #         Holder_df['ratio'] = Holder_df['name'].apply(lambda x: fuzz.ratio(i.lower(),x.lower()))
    #         Holder_df.sort_values(by='ratio', ascending=False, inplace=True)
    #         top_df = Holder_df.head()
    #         # get max ratio, it is used to determine if the match is close enough, above 85, to record as a likely match
    #         max_ratio = top_df['ratio'].max()
    #         top_df = top_df[['name']].T
    #         top_df.columns = ['MATCH_A','MATCH_B','MATCH_C','MATCH_D','MATCH_E']
    #         top_df['MAX_RATIO'] = [max_ratio]

    #         final_df = pd.concat((final_df,top_df))

    #     final_df['ORIGINAL'] = miss_companies_df['VALUE'].to_list()
    #     final_df['LIKELY_MATCH'] = final_df.apply(lambda x: x['MATCH_A'] if x['MAX_RATIO'] >= 85 else np.nan,axis=1)
    #     final_df = final_df[['ORIGINAL','LIKELY_MATCH','MATCH_A','MATCH_B','MATCH_C','MATCH_D','MATCH_E']]
    
    # final_df.to_csv(os.path.join(self.update_dir,'holder_comparisons.csv'),index=False)