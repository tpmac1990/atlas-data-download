...
def preformat_file(self):
    ...
    # for data_group in ['occurrence']:
        ...
        # deleteSecondofDuplicate_QLD_1(self) This went below 'combinePolygonsWithSameID_VIC'
...
def create_spatial_relation_files(self):
    ...
    # # print(self.geo_state_gdf.head())
    # self.geo_state_gdf.rename(columns={'code':'_id'},inplace=True)
    # self.geo_state_gdf.to_file(os.path.join(self.regions_dir,'State.shp'))
    # print('done')
...
def reduce_vertices(self):
    ''' remove all redundant points along polygon straight sides.
        ...
        ??? this can only update the Tenement file atm. The other files only need to be updated on first upload only and exist in the core directory. This needs to be moved eventually
    '''
    print('Reducing vertices in multipolygons')
    func_start = time.time()

    new_dir = os.path.join(self.output_dir,'new')

    # file_lst = ['GeologicalProvinceSpatial','GovernmentRegionSpatial','LocalGovernmentSpatial','StateSpatial','Tenement']
    file_lst = ['Tenement']

    for file in file_lst:
        
        path = os.path.join(new_dir,file+'.csv')
        ...
        if fileExist(path):
            ...
            df['geom'] = df['geom'].apply(lambda x: 'SRID=4202;%s'%(x))

            df.to_csv(path,index=False)

    print('cleaned Multipolygon data: %s' %(time_past(func_start,time.time())))




# df.rename(columns={'geom':'geom'},inplace=True)
# df.to_csv(os.path.join(self.input_dir,'temp',file+'_r.csv'),index=False)
# df.to_csv(os.path.join(self.input_dir,'temp',file+'_e.csv'),index=False)
# r_path = os.path.join(self.input_dir,'temp',file+'_r.csv')
# e_path = os.path.join(self.input_dir,'temp',file+'_e.csv')


# def separate_geom_fields(self):
#     ''' take the geom and ind field and put it in a separate table '''
#     print('Moving geom to its own table')
#     new_dir = os.path.join(self.output_dir,'new')

#     lst = [['Tenement','TenGeom'],['Occurrence','OccGeom']]

#     for x in lst:
#         print('Working on: %s'%(x[0]))
#         f_path = os.path.join(new_dir,'%s.csv'%(x[0]))
#         t_path = os.path.join(new_dir,'%s.csv'%(x[1]))

#         df = pd.read_csv(f_path)
#         df_1 = df[['ind','geom']].rename(columns={'ind':'ind_id'})
#         df = df.drop('geom',1)

#         df.to_csv(f_path,index=False)
#         df_1.to_csv(t_path,index=False)
    



# def reduce_pnts_in_poly(self):
