...
class CombineTitleDatasets:
    ...
    def format_file_to_combine(self,file,configs):
        ...
        # print(file)
        # print(self.Holder_df[self.Holder_df['name'].str.contains("Niagate, D")])
...
# occoriginalid_id)=(ENO0603863


        # # delete the ind field and rename the other to 'code' to merge with the core file
        # all_ids_df = new_related_ids_df.drop(new_related_ids_df.columns[0],axis=1)
        # all_ids_df.columns = ['_id']
        # all_ids_df['user_name'] = 'ss'
        # all_ids_df['valid_instance'] = True
        # all_ids_df['date_created'] = date.today()
        # all_ids_df['_id'] = all_ids_df['_id'].astype(str)
        # # open the current related_ids core file
        # id_df = pd.read_csv(path)
        # # concat both and delete if the value is a duplicate. Only the original and new ids will remain
        # OriginalID_df = pd.concat((id_df,all_ids_df)).drop_duplicates(subset='_id', keep="first")
        # # remove any null values
        # OriginalID_df = OriginalID_df[OriginalID_df['_id'] != 'nan']
        # OriginalID_df.to_csv(path,index=False)
...
#     print('Title data compilation: %s' %(time_past(func_start)))
...
#     print('Site data compilation: %s' %(time_past(func_start)))
